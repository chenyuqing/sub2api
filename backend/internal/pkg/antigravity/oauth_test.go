...
func TestConstants_值正确(t *testing.T) {
	...
	if got, err := getClientID(); err != nil || got != ClientID {
		t.Fatalf("getClientID 不匹配: got=%q err=%v want=%q", got, err, ClientID)
	}
	if got, err := getClientSecret(); err != nil || got != defaultClientSecret {
		t.Fatalf("getClientSecret 不匹配: got=%q err=%v want=%q", got, err, defaultClientSecret)
	}
