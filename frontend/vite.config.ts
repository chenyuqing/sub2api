...
export default defineConfig(({ mode }) => {
  ...
  const enableChecker = ['1', 'true'].includes(
    String(process.env.VITE_ENABLE_CHECKER ?? env.VITE_ENABLE_CHECKER ?? '').toLowerCase()
  )

  return {
    plugins: [vue(), injectPublicSettings(backendUrl)],
  resolve: {
    alias: {
