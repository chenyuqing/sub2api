...
const fetchLogs = async () => {
  ...
  try {
    ...
    appStore.showError(err?.response?.data?.detail || localText('系统日志加载失败', 'Failed to load system logs'))
  } finally {
    loading.value = false
  }
}
...
const saveRuntimeConfig = async () => {
  ...
  try {
    ...
    appStore.showSuccess(localText('日志运行时配置已生效', 'Runtime log settings saved'))
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to save runtime log config', err)
    appStore.showError(err?.response?.data?.detail || localText('保存日志配置失败', 'Failed to save log settings'))
  } finally {
...
const resetRuntimeConfig = async () => {
  const ok = window.confirm(localText('确认回滚为启动配置（env/yaml）并立即生效？', 'Revert to the startup config (env/yaml) and apply immediately?'))
  if (!ok) return
  ...
  try {
    ...
    runtimeConfig.stacktrace_level = saved.stacktrace_level
    runtimeConfig.retention_days = saved.retention_days
    appStore.showSuccess(localText('已回滚到启动日志配置', 'Reverted to startup log settings'))
    await fetchHealth()
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to reset runtime log config', err)
    appStore.showError(err?.response?.data?.detail || localText('回滚日志配置失败', 'Failed to revert log settings'))
  } finally {
    runtimeSaving.value = false
  }
}
...
const cleanupCurrentFilter = async () => {
  const ok = window.confirm(localText('确认按当前筛选条件清理系统日志？该操作不可撤销。', 'Clean system logs using the current filters? This cannot be undone.'))
  if (!ok) return
  try {
    const payload = {
    ...
    appStore.showSuccess(localText(`清理完成，删除 ${res.deleted || 0} 条日志`, `Cleanup complete, deleted ${res.deleted || 0} log entries`))
    page.value = 1
    await Promise.all([fetchLogs(), fetchHealth()])
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to cleanup logs', err)
    appStore.showError(err?.response?.data?.detail || localText('清理系统日志失败', 'Failed to clean system logs'))
  }
}
...
<template>
  <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-gray-900 dark:text-white">{{ localText('系统日志', 'System Logs') }}</h3>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ localText('默认按最新时间倒序，支持筛选搜索与按条件清理。', 'Sorted by newest first. Supports filtering, search, and cleanup by criteria.') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ localText('队列', 'Queue') }} {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-gray-100 px-2 py-1 text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ localText('写入', 'Written') }} {{ health.written_count }}</span>
        <span class="rounded-md bg-amber-100 px-2 py-1 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">{{ localText('丢弃', 'Dropped') }} {{ health.dropped_count }}</span>
        <span class="rounded-md bg-red-100 px-2 py-1 text-red-700 dark:bg-red-900/30 dark:text-red-300">{{ localText('失败', 'Failed') }} {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">{{ localText('运行时日志配置（实时生效）', 'Runtime log settings (apply immediately)') }}</div>
        <span v-if="runtimeLoading" class="text-xs text-gray-500">{{ localText('加载中...', 'Loading...') }}</span>
      </div>
    ...
    <div class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-5">
      ...
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ localText('组件', 'Component') }}
        <input v-model="filters.component" type="text" class="input mt-1" :placeholder="localText('如 http.access', 'e.g. http.access')" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ localText('请求 ID', 'Request ID') }}
        <input v-model="filters.request_id" type="text" class="input mt-1" />
      </label>
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ localText('客户端请求 ID', 'Client request ID') }}
      ...
      <label class="text-xs text-gray-600 dark:text-gray-300">
        {{ localText('关键词', 'Keyword') }}
        <input v-model="filters.q" type="text" class="input mt-1" :placeholder="localText('消息/request_id', 'message/request_id')" />
      </label>
    </div>

    <div class="mb-3 flex flex-wrap gap-2">
      <button type="button" class="btn btn-primary btn-sm" @click="applyFilters">{{ localText('查询', 'Search') }}</button>
      <button type="button" class="btn btn-secondary btn-sm" @click="resetFilters">{{ localText('重置', 'Reset') }}</button>
      <button type="button" class="btn btn-danger btn-sm" @click="cleanupCurrentFilter">{{ localText('按当前筛选清理', 'Clean current filter') }}</button>
      <button type="button" class="btn btn-secondary btn-sm" @click="fetchHealth">{{ localText('刷新健康指标', 'Refresh health metrics') }}</button>
    </div>

    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-gray-500">{{ localText('加载中...', 'Loading...') }}</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-gray-500">{{ localText('暂无系统日志', 'No system logs yet') }}</div>
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-gray-200 dark:divide-dark-700">
          <thead class="bg-gray-50 dark:bg-dark-900">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ localText('时间', 'Time') }}</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ localText('级别', 'Level') }}</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-gray-500">{{ localText('日志详细信息', 'Log details') }}</th>
            </tr>
          </thead>
